...
from harness.lib import docs_maintainer
...
def test_count_diff_lines_ignores_file_headers() -> None:
    ...
    assert docs_maintainer.count_diff_lines(diff) == 3


def test_count_diff_lines_empty() -> None:
    assert docs_maintainer.count_diff_lines("") == 0


def test_count_diff_lines_no_changes() -> None:
    diff = "--- a/x\n+++ b/x\n@@ -1,1 +1,1 @@\n only context\n"
    assert docs_maintainer.count_diff_lines(diff) == 0


def test_is_docs_path_accepts_doc_conventions() -> None:
    assert docs_maintainer.is_docs_path("README.md") is True
    assert docs_maintainer.is_docs_path("docs/payments/guide.md") is True
    assert docs_maintainer.is_docs_path("documentation/api/schema.json") is True


def test_is_docs_path_rejects_source_files() -> None:
    assert docs_maintainer.is_docs_path("services/payments/handler.py") is False
