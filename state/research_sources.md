...
- **Notes:** Protected source — never drop. Check CHANGELOG and releases, not just commits. v2.1.81: --bare flag (issue #63). SHA 6aadfbd unchanged 11th consecutive.
...
- **Notes:** Categories: Skills, Workflows, Tooling, Hooks, Slash-Commands, CLAUDE.md Files, Status Lines, Alternative Clients. Recent additions: parry (prompt injection scanner), RIPER workflow, ccpm (project management). Value is cross-reference for HORIZON SCAN, not direct patterns. All commits continue to be automated ticker/SVG updates — SHA e438b93 deep-dived, confirmed ticker-only (previous "first non-ticker" note was false positive).
...
- **Added:** 2026-03-20 (seed) | **Last deep:** 2026-03-24T20:21 | **Pattern hits:** 0
- **Notes:** Only actionable for security fixes or features that affect our site build. SHA d402485: host header validation fix (#16043, dev server security), Babel props for Preact (#15862). Previous: svgo update, turbo-run-affected, PR writer skill. 0 harness patterns across 4 deep-dives.
